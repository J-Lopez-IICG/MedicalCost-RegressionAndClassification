...
def create_classification_summary(
    ...
    summary += "| :--- | :---: |\n"
    summary += f"| Regresión Logística | {accuracy_log:.2f}% |\n"
    summary += f"| Support Vector Classifier (SVC) | {accuracy_best_svc:.2f}% |\n"
    summary += f"| XGBoost | {accuracy_best_xgb:.2f}% |\n"
    summary += f"| **Random Forest** | **{accuracy_best_rf:.2f}%** |\n\n"
    summary += "---\n\n"
    summary += (
        "> El modelo **Random Forest optimizado** es el campeón indiscutible de este análisis, logrando la mayor precisión con un **"
        + f"{accuracy_best_rf:.2f}%"
        + "**.\n\n"
    )
